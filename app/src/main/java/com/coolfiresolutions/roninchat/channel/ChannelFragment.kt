...
class ChannelFragment : BaseFragment() {
    ...
    lateinit var channel: Session
    lateinit var adapter: MessagesAdapter
    private var isWritingAttachmentToDisk = false
    private var attachment: File? = null

    interface ChannelListener {
        fun onEditChannelClicked(channel: Session)
    }

    companion object {
        val TAG = ChannelFragment::class.java.canonicalName!!

        private const val KEY_CHANNEL = "keyChannel"
        private const val SELECT_IMAGE = 4444

        fun newInstance(channel: Session): ChannelFragment {
            val bundle = Bundle()
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        tvDetailsToolbarText.text = channel.name
        listener = activity as ChannelListener

        adapter = MessagesAdapter(getApplication().networkProfile.userId)
        val layoutManager = LinearLayoutManager(activity)
        layoutManager.stackFromEnd = true
        rvMessages.layoutManager = layoutManager
        rvMessages.adapter = adapter

        getApplication().api.retrieveSessionMessages(getApplication().networkProfile.network, channel.id, roninMessageServiceCallbackListener)
    ...
    override fun onResume() {
        ...
        getApplication().localBroadcastManager.registerReceiver(roninMessageBroadcastReceiver, RoninMessageReceiver.getTextMessageIntentFilter())
    }

    override fun onPause() {
        super.onPause()
        getApplication().localBroadcastManager.unregisterReceiver(roninMessageBroadcastReceiver)
    }
    ...
    private fun initClickListeners() {
        ...
        ivToolbarDetails.setOnClickListener {
            listener.onEditChannelClicked(channel)
        }
    }
    ...
    private fun getChatMessage(messageAttachments: List<MessageAttachment>? = null): RoninMessage {
        ...
        //The Scope will be your target of the message and their object type (in this case we're sending a message to a "session")
        chatMessage.targets.add(ServerScope(channel.id, EntityType.SESSION.urlString))
    ...
    //Real-time callback
    private val roninMessageBroadcastReceiver = RoninMessageReceiver(object : RoninMessageReceiver.MessageReceiverListener {
        override fun onMessageReceived(message: RoninMessage) {
            //Check if the target is the current channel
            if (message.targets[0].id == channel.id) {
    ...
    //Callback for when we retrieve the initial list of users in the conversation
    private var usersServiceCallbackListener = object : UsersServiceCallback.UsersCallbackListener {
    ...
    private var roninMessageServiceCallbackListener = object : MessagesServiceCallback.MessagesCallbackListener {
